...
@implementation MathSinh

+ (instancetype)sinh
{
    static MathSinh *__weak s_weakRef;
    MathSinh *strongRef = s_weakRef;
    if (strongRef == nil) {
        s_weakRef = strongRef = [[MathSinh alloc] init];
    }
    return strongRef;
}
...
- (MathResult *)operateOnOperand:(MathResult *)operand
{
    ...
    decQuad resultValue;
    double _resultValue = sinh(_operandValue);
...
- (CGRect)rectWhenDrawWithContext:(MathDrawingContext *)context
{
    ...
    CGSize size = [@"sinh" sizeWithAttributes:attr];
...
- (CGRect)drawWithContext:(MathDrawingContext *)context
{
    ...
    NSDictionary *attr = @{NSFontAttributeName: font};
    CGSize size = [@"sinh" sizeWithAttributes:attr];
    [@"sinh" drawAtPoint:CGPointMake(context.origin.x, context.origin.y - font.ascender) withAttributes:attr];
    return CGRectMake(context.origin.x, context.origin.y - round(font.ascender), ceil(size.width), ceil(size.height));
}
