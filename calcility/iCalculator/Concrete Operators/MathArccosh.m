...
@implementation MathArccosh

+ (instancetype)arccosh
{
    static MathArccosh *__weak s_weakRef;
    MathArccosh *strongRef = s_weakRef;
    if (strongRef == nil) {
        s_weakRef = strongRef = [[MathArccosh alloc] init];
    }
    return strongRef;
}
...
- (MathResult *)operateOnOperand:(MathResult *)operand
{
    ...
    decQuad resultValue;
    double _resultValue = acosh(_operandValue);
...
- (CGRect)rectWhenDrawWithContext:(MathDrawingContext *)context
{
    ...
    CGSize size = [@"arccosh" sizeWithAttributes:attr];
...
- (CGRect)drawWithContext:(MathDrawingContext *)context
{
    ...
    NSDictionary *attr = @{NSFontAttributeName: font};
    CGSize size = [@"arccosh" sizeWithAttributes:attr];
    [@"arccosh" drawAtPoint:CGPointMake(context.origin.x, context.origin.y - font.ascender) withAttributes:attr];
    return CGRectMake(context.origin.x, context.origin.y - round(font.ascender), ceil(size.width), ceil(size.height));
}
