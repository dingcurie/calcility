...
- (MathResult *)operate
{
    ...
    if (_nValue >= 0 && _kValue >= 0 && trunc(_nValue) == _nValue && trunc(_kValue) == _kValue) {
        if (_kValue <= _nValue) {
            double _resultValue = round(tgamma(1 + _nValue) / (tgamma(1 + _kValue) * tgamma(1 + _nValue - _kValue)));
...
- (CGPoint)originOfSubexpressionAtIndex:(NSUInteger)subexpressionIndex whenDrawWithContext:(MathDrawingContext *)context
{
    ...
    CGFloat nameWidth = [@"C" sizeWithAttributes:attr].width;
...
- (CGRect)drawWithContext:(MathDrawingContext *)context
{
    ...
    [@"C" drawAtPoint:CGPointMake(context.origin.x, context.origin.y - font.ascender) withAttributes:attr];
