...
@implementation MathCosh

+ (instancetype)cosh
{
    static MathCosh *__weak s_weakRef;
    MathCosh *strongRef = s_weakRef;
    if (strongRef == nil) {
        s_weakRef = strongRef = [[MathCosh alloc] init];
    }
    return strongRef;
}
...
- (MathResult *)operateOnOperand:(MathResult *)operand
{
    ...
    decQuad resultValue;
    double _resultValue = cosh(_operandValue);
...
- (CGRect)rectWhenDrawWithContext:(MathDrawingContext *)context
{
    ...
    CGSize size = [@"cosh" sizeWithAttributes:attr];
...
- (CGRect)drawWithContext:(MathDrawingContext *)context
{
    ...
    NSDictionary *attr = @{NSFontAttributeName: font};
    CGSize size = [@"cosh" sizeWithAttributes:attr];
    [@"cosh" drawAtPoint:CGPointMake(context.origin.x, context.origin.y - font.ascender) withAttributes:attr];
    return CGRectMake(context.origin.x, context.origin.y - round(font.ascender), ceil(size.width), ceil(size.height));
}
